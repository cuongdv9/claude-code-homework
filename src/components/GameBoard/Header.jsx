...
function ProgressBar({ questionIndex, history }) {
  return (
    <div
    ...
    >
      {MONEY_LADDER.map((_, i) => {
        const result = history[i]; // 'correct' | 'wrong' | undefined
        const current = i === questionIndex;
        return (
          <div
            key={i}
            className={`progress-seg ${result ? result : ""} ${current ? "current" : ""}`}
          />
        );
      })}
    </div>
...
export default function Header({
  ...
  return (
    <div className="header-wrap">
      <ProgressBar questionIndex={questionIndex} history={history} />
